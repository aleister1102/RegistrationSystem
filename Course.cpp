...
Course Course_Input()
{
	...
	do
	{
		...
		cout << "\t\t Teacher Name: ";
		getline(cin, new_course.teacher);
		cout << "\t\t Number of credits: ";
		cin >> new_course.cre;
		cout << "\t\t Maximum students: ";
		cin >> new_course.capacity;
...
bool Course_Delete(string faculty)
{
	...
	while (!f.eof())
	{
		...
		getline(f, reader);
		if (reader == "")
			continue; //Chuỗi rỗng thì bỏ qua
...
void Course_Update(string semester_path, string faculty)
{
	...
	if (teacher_temp == c.teacher)
		return; //Tên giáo viên không đổi thì out
