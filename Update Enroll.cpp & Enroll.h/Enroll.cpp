...
void Get_date(date& d1, date& d2, string semester, string path)
{
	fstream f;
	if (!File_Exist(path + semester))
	{
		cout << "This semester haven't been created !!!" << endl;
		system("pause");
		return;
	}
	f.open(path + semester, ios::in | ios::out);
	while (!f.eof())
	{
		string s1, s2;
		f >> s1;
...
bool Check_Sems(date dmy, string semester, string path)
{
	date d1, d2;
	Get_date(d1, d2, semester, path);
	if (dmy.year == d1.year)
	{
...
string Get_Semester(string path, date dmy)
{
	int i = 0;
	//int check = false;
	...
	while (i <= 2)
	{
		bool k = Check_Sems(dmy, Extension(s[i], 1), path);
		if (k)
		{
			return s[i];
		}
...
int Enroll_Dis(Student sv,date dmy)
{
	...
	path += years+"\\";
	string sems = Get_Semester(path, dmy);
	if (sems == "OUT")
	{
	...
	string filename = Make_Path(path, sems);
	int n;
